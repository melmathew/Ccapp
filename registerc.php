...
echo "Connected successfully";
	...
	
   {
			...
			$sqlc="SELECT chno from info";
			$sql = "SELECT ano FROM info WHERE ano='$ano'";
			$query_runc=$conn->query($sqlc);
			$query_run=$conn->query($sql);
			$numcount=$query_runc->num_rows;
			...
			$esql = "SELECT email FROM info WHERE email='$email'";
		 ...
		 if ($equery_run->num_rows ==0)
		 {
			if($query_run->num_rows == 0)
			{
				...
				$query = "INSERT INTO info (fname,lname,gr,email,mno,ano,id,chno) 
				VALUES('$fname','$lname','$gr','$email','$mno','$ano','$id','$numcount')";
		 ...
		 <div>
		  <div>
		   ...
		   <div class="mdiv" style="height:380px;">
                              ...
                              <center> 
                              <span><h1 class="rotateIn">Candidate Registration</h1></span>
                              <form method="post">
                               ...
                               <span class="txt">Last Name :</span><br>
								...
								id="eid1" placeholder="Enter your email"
								size="50" required
